  ...
  const checkForErrors = debounce(
    (_, manual = false) => {
      ...
      errors.forEach((line) => {
        ...
        // check if we need adjustment for white space
        const textLine = document.lineAt(error.line).text;
        const adjust = (textLine.match(/^(\s+)/) || [, ''])[1].length;
        let range = new vscode.Range(
          ...
          error.end
            ? error.end + adjust
            : document.lineAt(error.line).text.length
        );
