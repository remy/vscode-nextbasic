  ...
  return vscode.commands.registerCommand('nextbasic.run', async () => {
    try {
      ...
      if (text.includes('#bank')) {
        ...
        banksSent = res.bankSplits.length;

        for (let i = 0; i < res.bankSplits.length; i++) {
          const bank = res.bankSplits[i];
          const src = join(tmpdir(), sep, bank.filename);
          const dest = `/devel/${bank.filename}`;
          const commandLine = `${hdfmonkey} put "${nextImagePath}" "${src}" ${dest}`;
          await exec(commandLine, { cwd: extensionPath });
        }
      }
      ...
      try {
        launching = false;
        let msg = 'Sent index.bas to cspect';
        if (stripComments) {
          msg += ' (comments stripped)';
        }
        if (banksSent) {
          msg += ` and ${banksSent} bank${banksSent > 1 ? 's' : ''}`;
        }
